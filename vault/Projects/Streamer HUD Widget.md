...
- **Technical condition:** the particle-sphere core (`C:\Jarvis\orb`) is reusable, but the Claude Code chat backend, hardcoded paths/token, and personal panels are not — must be dropped/rebuilt for a generic product. Highest-priority open question: whether the audio-reactivity (currently gated behind a real user click) works unattended inside an actual OBS browser source — unverified, needs prototyping.
- **Current status:** market/technical validation done, no build work started. Next step is prototyping audio-reactivity in real OBS to resolve the autoplay/click-gate risk before committing to distribution model or price.
