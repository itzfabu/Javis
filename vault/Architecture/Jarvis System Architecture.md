...
Architecture note for the Jarvis personal AI assistant system, imported verbatim from the prior vault's `Jarvis-README.md` on 2026-07-29. Covers folder structure, subagents, hooks, the orb dashboard, connectors, cloud routines, security posture, and known gotchas. Migration note: the source file had mojibake corruption (em-dashes and box-drawing tree characters had been double-encoded); repaired during import by re-decoding through windows-1252, verified line-by-line against the original before writing here. Body left otherwise verbatim, including em-dashes, per explicit instruction at import time - not yet passed through this vault's normal ASCII-punctuation house style.
...
Note: this folder structure describes the source `C:\Jarvis` layout, including its `vault\` PARA sub-structure (Projects/Areas/Resources/Archive) — this is a description of the *old* vault, not of the vault this note now lives in.
